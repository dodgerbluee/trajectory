...
/**
 * Development Mock Data Seeder (Simplified)
 * 
  ...
  try {
    ...
    for (const child of children) {
      ...
      const illnesses = [
        ...
        { type: 'chicken_pox', startAge: 5, duration: 7, severity: 2, description: 'Childhood illness with rash' },
      ];
      ...
      // Injuries
      const injuries = [
        { age: 2, type: 'fall', description: 'Fall from playground equipment' },
        { age: 3, type: 'cut', description: 'Cut from sharp object' },
        { age: 5, type: 'sports', description: 'Sports injury during soccer' },
        { age: 7, type: 'burn', description: 'Minor burn from hot surface' },
      ];
      
      for (const injury of injuries) {
        if (injury.age <= ageInYears) {
          const injuryDate = new Date(currentTime.getTime() - (ageInYears - injury.age) * 365.25 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
          await createVisit(child.id, 'injury', injuryDate);
          await createInjuryNote(visitId, injury.description);
        }
      }
    ...
    console.log('📊 Created comprehensive health dataset for 5 children');
    console.log('👤 Users: buckley@dev.local /', DEFAULT_PASSWORD);
...
async function createVisit(childId, visitType, visitDate) {
  ...
  const result = await pool.query('SELECT id FROM visits WHERE id = $1', [visitId]);
...
async function createInjuryNote(visitId, description) {
  const noteId = uuidv4();
  await pool.query(`
    INSERT INTO visit_notes (id, visit_id, notes, created_at, updated_at) 
    VALUES ($1, $2, $3, NOW(), NOW())

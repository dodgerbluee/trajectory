/**
 * PersonChipRow – horizontally scrollable row of person avatars (story-rail style).
 *
 * Used at the top of the mobile home feed to scope the rest of the feed to a
 * single person. When `selectedId` is null the feed shows everyone; selecting a
 * person highlights their avatar with a colored ring. The "Show all" affordance
 ...
 *
 * Long-press (≥500ms) on a chip navigates to that person's detail page; a
...
import styles from './PersonChipRow.module.css';
...
function PersonChipRow({ peopleList, selectedId, onSelect }: PersonChipRowProps) {
  ...
  const orderedPeople = useMemo(
    () => sortPeopleWithAdultsLast(peopleList),
    [peopleList]
  );

  if (orderedPeople.length === 0) return null;
  ...
  const startLongPress = (personId: number, x: number, y: number) => {
    clearTimer();
  ...
  return (
    <div className={styles.wrap}>
      <div
        className={`${styles.row} ${orderedPeople.length > 3 ? styles.rowCentered : ''}`}
        role="tablist"
        ...
        {orderedPeople.map((person) => {
          ...
          return (
            <button
              ...
              onPointerDown={(e) => startLongPress(person.id, e.clientX, e.clientY)}
              ...
              onClick={(e) => {
                ...
                onSelect(isActive ? null : person.id);
              }}
            >

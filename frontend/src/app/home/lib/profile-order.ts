/**
 * Helpers for ordering profile lists (children + adults) shown on Home.
 *
 * Adults (18+) are pushed to the end of the avatar row / family grid, so the
 * primary focus stays on children. Within each group (kids, adults) the
...
export function isAdultProfile(child: Child): boolean {
  if (!child.date_of_birth) return false;
  const { years } = calculateAge(child.date_of_birth);
  return years >= ADULT_AGE_YEARS;
}

/**
 * Returns a new array where adults (18+) are moved to the end while children
 * retain their original relative order. Does not mutate the input.
 */
export function sortChildrenWithAdultsLast(children: Child[]): Child[] {

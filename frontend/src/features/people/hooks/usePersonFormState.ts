/**
 * usePersonFormState — single source of truth for the Add/Edit Person form.
 *
 * Owns:
 ...
 *
 * Both AddPersonPage and EditPersonPage use this so the field set, validation
...
export function emptyChildFormData(): PersonFormData {
  return {
    name: '',
...
export function personFormDataFromPerson(person: Person): PersonFormData {
  return {
    name: person.name,
    date_of_birth: formatDateForInput(person.date_of_birth),
    gender: person.gender,
    notes: person.notes || '',
    due_date: person.due_date ? formatDateForInput(person.due_date) : '',
    birth_weight: person.birth_weight?.toString() || '',
    birth_weight_ounces: person.birth_weight_ounces?.toString() || '',
    birth_height: person.birth_height?.toString() || '',
  };
}
 ...
 */
function normalizePayload(formData: PersonFormData) {
  const ouncesParsed =
...
interface UseChildFormStateOptions {
  /** Initial form data (e.g., from a fetched Person for edit, or empty for add). */
  initial?: PersonFormData;
}

export function usePersonFormState(options: UseChildFormStateOptions = {}) {
  ...
  /** Replace all form data (e.g., when an async fetch resolves in EditPersonPage). */
  const reset = useCallback((next: PersonFormData) => {
    setFormData(next);
    setErrors({});
  }, []);
  ...
  const toCreatePayload = useCallback(
    (extras?: { family_id?: number }): CreatePersonInput => ({
  ...
  const toUpdatePayload = useCallback(
    (): UpdatePersonInput => normalizePayload(formData),
    [formData],
  );

 ...
 * want to track their own health alongside the children/family members they
 * track. Shown on HomePage when:
 *   - user is authenticated
 *   - user.hasSelfChild === false
...
import { meApi, childrenApi, ApiClientError } from '@lib/api-client';
...
import { GenderToggle } from '@features/children/components';
import {
  emptyChildFormData,
  useChildFormState,
...
export type SelfRecordPromptResult =
  | { kind: 'created'; childId: number }
  | { kind: 'created-avatar-failed'; childId: number; avatarError: string }
  | { kind: 'dismissed' };
...
function SelfRecordPromptModal({ onResolved }: Props) {
  ...
  const { formData, updateField, errors, clearError, validate } = useChildFormState({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (avatarFile) {
        try {
          await childrenApi.uploadAvatar(childId, avatarFile);
        } catch (avatarError) {
          ...
          const message =
            avatarError instanceof ApiClientError
              ? avatarError.message
              : 'Photo upload failed';
          onResolved({ kind: 'created-avatar-failed', childId, avatarError: message });
          return;
        }
      }

      onResolved({ kind: 'created', childId });
    } catch (error) {
